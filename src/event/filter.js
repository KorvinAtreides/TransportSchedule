...
export default function filterIn(btn){
    ...
    for(let btn2 of btnFiltrs){btn2.style.backgroundColor ='white'}
    btn.style.backgroundColor ='rgb(89, 187, 187)';
    let lists = document.querySelectorAll("li");
    if (btn.id == 'btnFiltr1'){
      for(let li of lists){li.style.display = "block";}
    }
    if (btn.id == 'btnFiltr2'){
      let today = new Date();
      for(let li of lists){
        ...
        let dateGo=newRace.date;
        let timeGo=String(newRace.timeStart);
        let hour = Number((timeGo[0]+timeGo[1]));
        let min = Number((timeGo[3]+timeGo[4]));
        if ((Date.parse(today)-Date.parse(dateGo)-((hour-3)*60+min)*60*1000)>0){
